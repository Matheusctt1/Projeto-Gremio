...
def mostrarFormDados(df):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Cadastrar Jogo", "Cadastrar Posições", "Cadastrar Coletivo", "Cadastrar Adversário"])
    ...
    with tab2:
        if st.sidebar.button("Reiniciar Cadastro"):
            limparVariaveis()
        st.subheader('Cadastrar Posições', divider='gray')
        column1, column2 = st.columns(2)
        
        with column1:
            clickButtonKey = 'clickButton'
            if clickButtonKey not in st.session_state:
                st.session_state[clickButtonKey] = 0
            aparecerNome = listaAtletasFiltered
            if st.session_state[clickButtonKey] < len(listaAtletasFiltered):
                #st.markdown(f'Atleta: {aparecerNome[st.session_state[clickButtonKey]]}')
                st.selectbox('Atletas: ', options=aparecerNome)
                posicao = st.selectbox('Posições:', options=posicoes)
                if st.button("Cadastrar Posição"):
                    cadastrarPosicao(posicao)

                    st.session_state[clickButtonKey] += 1
            else:
                st.success("Todos os atletas foram cadastrados.")

            
            #show_table = st.checkbox("Mostrar Tabela Individual")
            #if show_table:
                #st.write(df.head(25))

    #with tab3:
        #st.subheader('Informações Goleiros', divider='gray')
        #column3, column4 = st.columns(2)
        #with column3:
            #st.selectbox('Goleiros', options=listaGoleiros)
        #with column4:
            #show_table_goleiros = st.checkbox("Mostrar Tabela Goleiros")
        #if show_table_goleiros:
            #st.write(df2.head(17))
    ...
    if button_dadosJogo:
        ...
        with st.status("Cadastrando Jogo..."):
            filtrarDataFrame.filtrarDataFrame(infoJogo, infoColetivo, listaAtletasFiltered, listaGoleiros, df, session_state.listaPosicao)
