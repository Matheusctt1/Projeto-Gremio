...
def cadastrar():
    ...
    with tab2:
        with st.container():
            ...
            uploaded_fileGo = st.file_uploader("Insira a foto do Goleiro", type=['jpg', 'png'])
            if uploaded_fileGo is not None:
               
                file_bytesGo = uploaded_fileGo.read()
                file_nameGo = uploaded_fileGo.name
            ...
            if button:
                with st.spinner('Um momento..'):
                    ...
                    if verificarBDGoleiro(nomeGoleiro, conn):
                        ...
                        controleGoleiro.cadastrarGoleiro(goleiro.goleiro(data_cadastro, nomeGoleiro, file_bytesGo))
