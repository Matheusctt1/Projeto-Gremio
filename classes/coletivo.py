class coletivo:
    def __init__(self, adversarioData, nivelJogo, dataJogo, campeonato, categoria, referencia, tempoTotalJogo
                ,gol, finaGol, finaFora, finaBloq, assistFina, assistGol, chanceClara
                ,interceptacao, bloqFina, desarmeCerto, desarmeErrado, dribleCerto, perdaBola, passeVert, passeVertErrado, passeCerto, passeErra, passeLongo
                ,passeLongoErra, cruzamento, cruzamentoErra, vitoriaAereo, derrotaAereo, MCD, MCO, posseGeral
                ,posseOfensiva,posseMeio, posseDefensiva, finalizacaoOrg, finalizacaoTR, finalizacaoBP
                ):
                ...
                self.passeVertErrado = passeVertErrado
                self.passeCerto = passeCerto
                self.passeErra = passeErra
                ...
                self.MCD = MCD
                self.MCO = MCO
                self.posseGeral = posseGeral
                self.posseOfensiva = posseOfensiva
                self.posseMeio = posseMeio
                self.posseDefensiva = posseDefensiva
                self.finalizacaoOrg = finalizacaoOrg
                self.finalizacaoTR = finalizacaoTR
                self.finalizacaoBP = finalizacaoBP